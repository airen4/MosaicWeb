...
<body>
	...
	<br>
	<button class="btn btn-warning">getJSON dept + handlebars</button>
	<button class="btn btn-warning">getJSON depts + handlebars</button>
	...
	<script type="text/javascript">
		...
		$('button').eq(1).on('click', function() {
					...
					var lis = "";
						for (var i = 0; i < data.length; i++) {
							lis += '<li class="list-group-item list-group-item-danger">' 
							    + data[i].deptno
								+ ", "
								+ data[i].dname
								+ ", "
...
<script id="temp2" type="text/xxx-mytemplate">
	{{#each .}}
	<li class="list-group-item list-group-item-success">{{deptno}}, {{dname}}, {{loc}}</li>
	{{/each}}
